...
EN = [
    ...
    ("li", "Learn-tol cap — entry box (default 0.15) that caps the learner's effective fuzzy tolerance independent of s$tNtype. See section 5. Set to 0 to disable the cap and use the raw TM value."),
    ...
    ("p", "Learn-tolerance cap: there are two tolerances — the one OpenHoldem uses at scrape time (the raw s$tNtype from the TM, e.g. 0.35) and the one ohlearn uses to decide 'this glyph is already covered, don't prompt for it' (capped at 0.15 by default). When the TM is lenient, the cap keeps the learner stricter so additional bitmap variants are captured. Scrapers configured to run stricter than the TM — for example OpenScrape at 0.15 with a 0.35 TM — then match reliably because every visual variation within 0.15 of some stored glyph has been saved."),
    ("p", "Effective learning threshold = min(s$tNtype, LEARN_FUZZY_CAP). Cap = 0 disables the cap (falls back to raw TM behaviour). The GUI log prefixes the effective value in brackets when it diverges, e.g. tol=0.15(TM=0.35)."),

    ("h2", "6. Miss-scrape prevention"),
    ("p", "Several layers detect and prevent miss-scrapes:"),
    ("li", "Tiny glyph filter: isolated small segments (commas, dots, artifacts) are only offered for learning when accompanied by normal-sized segments (digits) in the same region. Prevents learning noise as separator characters."),
    ("li", "Pre-save validation: before the label dialog appears, the system checks cross-similarity between the new glyph and existing fonts. If a separator bitmap is too similar to a digit (or vice versa), a red 'MISS-SCRAPE RISK' warning is shown with a recommendation to skip or discard."),
    ("li", "Post-learn validation: immediately after each new glyph is learned, a background thread captures a fresh frame and trial-scrapes all active T regions using OH-faithful font scanning (DoPlainFontScan simulation with per-font y bounds). Suspicious results are flagged."),
    ("li", "Periodic validation: every 60 seconds during learning, the same trial-scrape runs across all active regions. Detects: separators without digits on both sides, invalid thousands grouping (e.g. 2,6,601 or 4,,824), and adjacent mixed separators."),
    ("li", "Card region validation: card regions must produce a single character or '10' (ten). Multi-char results like 't9' or '15' are flagged as suspicious, except for the valid '10' representation."),
    ("p", "When a suspicious bitmap is found, a ValidationDialog shows the bitmap in context with the scraped text and the specific issue. Options: Delete bitmap (removes from TM), Keep (dismiss for session), Skip (dismiss)."),

    ("h2", "7. Card regions — tens"),
    ("p", "When Start Learning is clicked with card regions selected, an info dialog reminds the user about labeling tens:"),
    ("li", "For tens ('10'): label the '1' and '0' bitmaps as separate characters, then verify in OpenScrape that the card region scrapes correctly."),
    ("li", "Do NOT use 't' or 'T' — OpenHoldem expects individual '1' and '0' bitmaps."),
    ("li", "For face cards (J/Q/K/A): use single lowercase letter (j, q, k, a)."),

    ("h2", "8. Tablemap format notes"),
    ...
    ("li", "learn.py — observation pipeline, fuzzy match, image tolerance, auto-tune, prune, miss-scrape validation."),
    ("li", "tm.py — .tm / .tmn / .osdb2 parser and byte-faithful writer."),
    ("li", "gui.py — Tkinter UI: region list, learning loop, label/validation dialogs, logging."),
...
CZ = [
    ...
    ("li", "Learn-tol cap — pole (default 0.15), které capne efektivní fuzzy toleranci learneru nezávisle na s$tNtype. Viz sekce 5. Nastav na 0 pro vypnutí capu (učí se na raw TM hodnotě)."),
    ...
    ("p", "Efektivní learning práh = min(s$tNtype, LEARN_FUZZY_CAP). Cap = 0 ho vypne (chová se jako raw TM). Log v GUI ukazuje efektivní hodnotu v závorce, když se liší od TM, např. tol=0.15(TM=0.35)."),

    ("h2", "6. Prevence miss-scrapů"),
    ("p", "Několik vrstev detekuje a předchází miss-scrapům:"),
    ("li", "Filtr drobných glyphů: izolované malé segmenty (čárky, tečky, artefakty) se nabídnou k učení jen když jsou doprovázeny normálně velkými segmenty (číslicemi) ve stejném regionu. Zabraňuje učení šumu jako separátorů."),
    ("li", "Validace před uložením: před zobrazením label dialogu systém zkontroluje křížovou podobnost mezi novým glyphem a existujícími fonty. Pokud je bitmapa separátoru příliš podobná číslici (nebo naopak), zobrazí se červené varování 'MISS-SCRAPE RISK' s doporučením přeskočit nebo zahodit."),
    ("li", "Validace po naučení: ihned po naučení každého nového glyphu background thread sejme čerstvý snímek a provede trial-scrape všech aktivních T regionů pomocí OH-věrného font scanningu (simulace DoPlainFontScan s per-font y bounds). Podezřelé výsledky se nahlásí."),
    ("li", "Periodická validace: každých 60 sekund během učení probíhá stejný trial-scrape přes všechny aktivní regiony. Detekuje: separátory bez číslic po obou stranách, nevalidní tisícové skupiny (např. 2,6,601 nebo 4,,824), sousedící smíšené separátory."),
    ("li", "Validace card regionů: card regiony musí produkovat jeden znak nebo '10' (desítka). Víceznakové výsledky jako 't9' nebo '15' se označí jako podezřelé, s výjimkou validní reprezentace '10'."),
    ("p", "Když se najde podezřelá bitmapa, ValidationDialog ukáže bitmapu v kontextu se scrapovaným textem a konkrétním problémem. Možnosti: Delete bitmap (smazat z TM), Keep (zamítnout pro session), Skip (přeskočit)."),

    ("h2", "7. Card regiony — desítky"),
    ("p", "Když se klikne na Start Learning s vybranými card regiony, zobrazí se informační dialog připomínající pravidla pro desítky:"),
    ("li", "Pro desítky ('10'): označit bitmapy '1' a '0' jako samostatné znaky, pak ověřit v OpenScrape že card region scrapuje správně."),
    ("li", "NEPOUŽÍVAT 't' nebo 'T' — OpenHoldem očekává jednotlivé bitmapy '1' a '0'."),
    ("li", "Pro figurky (J/Q/K/A): použít malé písmeno (j, q, k, a)."),

    ("h2", "8. Formát tablemapy"),
    ...
    ("li", "learn.py — observation pipeline, fuzzy match, image tolerance, auto-tune, prune, validace miss-scrapů."),
    ("li", "tm.py — parser a bajt-přesný writer .tm / .tmn / .osdb2."),
    ("li", "gui.py — Tkinter UI: region list, learning loop, label/validační dialogy, logging."),
